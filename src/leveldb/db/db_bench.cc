...
class Benchmark {
 private:
  ...
  void PrintEnvironment() {
    fprintf(stderr, "LevelDB:    version \n");
...
int main(int argc, char** argv) {
  FLAGS_write_buffer_size = pdlfs::Options().write_buffer_size;
  FLAGS_block_size = pdlfs::Options().block_size;
...
int main(int argc, char** argv) {
  if (argc > 1 && std::string(argv[1]) == "--benchmark") {
    ::pdlfs::BM_LogAndApply(1000, 1);
    ::pdlfs::BM_LogAndApply(1000, 100);
    ::pdlfs::BM_LogAndApply(1000, 10000);
    ::pdlfs::BM_LogAndApply(100, 100000);
    return 0;
  }
