...
class KeyValOffsetIterator : public Iterator {
 public:
	explicit KeyValOffsetIterator(KeyValOffVec* k_voff) : vec_(k_voff),
																												iter_(k_voff->begin()) {
  ...
  virtual Status status() const { return Status::OK(); }iter_
 private:
	KeyValOffVec* vec_;
...
class VLogColumnImpl : public Column {
 public:
  explicit VLogColumnImpl(const Options& raw_options, const std::string& columnname) :
  ...
  Status Get(const ReadOptions& options, const LookupKey& lkey,
                     Buffer* result);

  Status BulkInsertVLog(KeyValOffVec *const kvoff_vec, const std::string& fname, Iterator* iter);
