...
class YCSBColumnarDB: public DB {
 ...
 public:
  ...
  explicit YCSBColumnarDB(pdlfs::ColumnStyle style) : dbname_(pdlfs::test::TmpDir() + "/ycsb_test") {
    ...
    Status s =
        pdlfs::ColumnarDB::Open(options_, dbname_, &column_selector_, styles, 1, &db_);
    ASSERT_OK(s);
  }
  ...
  int Scan(const std::string &table, const std::string &key,
    ...
    for (iter->SeekToFirst(); i < len && iter->Valid(); iter->Next(), ++i) {
    	...
    	result[result.size()-1].push_back(std::make_pair(FIELDNAME, iter->value().ToString()));
    }
    delete iter;
		return DB::kOK;
  }
